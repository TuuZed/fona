 ...
 * written to NVRAM. See {@link #open(String, Integer)} for
 * a details.</li>
 * </ul>
 */
public class Fona implements FonaEventHandler {
     ...
     * auto-baud,
     * {@link #simWaitForReady(int, Ready)} will
     ...
     * {@link #emailSMTPLogin(String, Integer)} or
     * {@link #emailSMTPLogin(String, Integer, String, String)}
     *
     ...
     * POP3 server with
     * {@link #emailPOP3Login(String, Integer, String, String)}
     *
     ...
     * use
     * {@link com.angryelectron.fona.FonaSerial#open(String, Integer)}.
     *
     ...
     * {@link FonaEventHandler#onNetworkStatusChange(Network) }.
     ...
     * <ul>
     * <li>{@link #simWaitForReady(int, Ready)
     * }</li>
     * <li>{@link FonaEventHandler#onSerialReady() }</li>
     * <li>{@link FonaEventHandler#onNetworkStatusChange(Network)}</li>
     * </ul>
     * <p>
     ...
     * {@link #simWaitForReady(int, Ready)}.
     *
     * @throws FonaException if fona is not responding
     */
     ...
     * {@link #open(String, Integer)} for an important note
     ...
     *
     * @return 手机号码
     * @throws FonaException 如果读取不到手机号码
     */
    public String simNumber() throws FonaException {
        String response = serial.atCommand("AT+CNUM", 10000);
        if (response.contains("ERROR")) {
            throw new FonaException("Reading number failed: " + response);
        }
        Pattern pattern = Pattern.compile("([1-9][0-9]{11,})", Pattern.DOTALL);
        Matcher matcher = pattern.matcher(response);
        if (!matcher.find()) {
            throw new FonaException("Reading sim number failed: " + response);
        }
        return matcher.group(1);
    }
